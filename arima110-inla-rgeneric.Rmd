---
title: "Implementing the $\\text{ARIMA}(1, 1, 0)$ model using `rgeneric`"
author:
...
# `R-INLA` $\text{AR}(1)$ example using `rgeneric`
...
# `R-INLA` $\text{ARIMA}(1, 1, 0)$ using `rgeneric`

$\alpha \sim \text{ARIMA}(1, 1, 0)$ is equivalent to $\gamma ~ AR(1)$ where $\gamma = D\alpha$ and $D$ is the first order difference matrix.
Let $\tilde Q$ be the $\text{AR}(1)$ precision matrix.
Then $\gamma' \tilde Q \gamma = \alpha' D' \tilde Q D \alpha$.
Thus the precision matrix for $\text{ARIMA}(1, 1, 0)$ is simply $Q = D' \tilde Q D$.
$\det(Q) = n \det(\tilde Q)$. 
...
The parameter `mean = <>` to `arima.sim()` passes a mean value to `rnorm()` used to simulate the innovations.
To obtain a process with marginal slope `c`, pass the value `mean = c * (1 - rho) / sqrt(1 - rho^2)`.
